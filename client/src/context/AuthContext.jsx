...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // 1. Fire context login layer (returns the raw data payload directly)
      const responseData = await login(form.email, form.password);
      
      // 2. Resolve target session token directly out of the payload
      if (responseData && responseData.token) {
        localStorage.setItem('token', responseData.token);
        navigate('/home');
      } else {
        setError('Authentication succeeded, but no access token was recovered.');
      }
    } catch (err) {
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="bg-white border border-[#e8e4dc] rounded-2xl p-6 sm:p-8 shadow-xl shadow-black/5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="label text-xs font-semibold text-[#555]">Email address</label>
              ...
              <div>
                <label className="label text-xs font-semibold text-[#555]">Password</label>
